(function(){
  ...
  function syncBottomNav(){
    const nav=[...document.querySelectorAll('.fixed.bottom-0')].find(el=>{
      const r=el.getBoundingClientRect();
      return r.height>0 && r.bottom>=window.innerHeight-2;
    });
    document.documentElement.dataset.nsBottomNav=nav?'1':'0';
  }
  function run(){setState();syncBottomNav();enhanceGuest();touchTargets();cleanStatus();}
  ...
  new MutationObserver(()=>{syncBottomNav();enhanceGuest();touchTargets();cleanStatus();}).observe(document.body,{childList:true,subtree:true});
  window.addEventListener('resize',syncBottomNav,{passive:true});
  setInterval(()=>{setState();syncBottomNav();},1500);
})();
